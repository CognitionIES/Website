...
// This component shows a list of useful links
...
export default function UsefulLinks() {
  const { TITLE, ITEMS } = FOOTER_CONSTANTS.USEFUL_LINKS;
      ...
      <ul className="space-y-2">
        {ITEMS.map(([title, href]) => (
          ...
          >
            ...
            >
              <ChevronRight className="text-white opacity-70 w-6 h-6 group-hover:text-[#0098AF] transition-colors duration-300" />
              {title}
            </Link>
            {/* Hover dot */}
            <motion.span
